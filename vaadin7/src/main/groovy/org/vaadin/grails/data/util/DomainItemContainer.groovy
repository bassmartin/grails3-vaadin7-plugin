...
implements Filterable, Sortable, ValueChangeListener, PropertySetChangeNotifier {
    ...
    final Set<Object> propertyIds = new HashSet()
    ...
    protected DomainItem<T> createDomainItem(T object) {
        def item = (object == null ? null : new DomainItem<T>(object))
        if (item) {
            propertyIds.each { Object propertyId ->
                if (!item.itemPropertyIds.contains(propertyId)) {
...
//        TODO removeAllValueChangeListeners(item)
    ...
    boolean addContainerProperty(Object propertyId, VaadinPropertyDescriptor<T> propertyDescriptor) {
